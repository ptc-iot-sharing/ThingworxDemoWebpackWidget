...
import { TWWidgetDefinition, autoResizable, description, property, defaultValue, bindingTarget, service, event, bindingSource, nonEditable, willSet, didSet } from 'typescriptwebpacksupport/widgetidesupport';
...
class DemoWebpackWidget extends TWComposerWidget {
    ...
    @property('STRING', bindingTarget, defaultValue('my value'), willSet('valueWillSet'), didSet('valueDidSet')) value: string;
    ...
    @property('NUMBER', bindingSource, nonEditable) clickedAmount: number;
    ...
    @property('STRING', bindingTarget, defaultValue('Invoked via decorator')) clickMessage: string; 
